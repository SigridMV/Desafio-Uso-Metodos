...
var pacientes_traumatologia = traumatologia.concat(nuevos_traumatologia);
...
document.write(
  `<p>Cantidad de atenciones para Traumatología: ${pacientes_traumatologia.length}</p>`
);
...
document.write(
  `<p>Primera atencion: ${pacientes_traumatologia[0].paciente} - ${
    pacientes_traumatologia[0].prevision
  } | Última atención: ${
    pacientes_traumatologia[pacientes_traumatologia.length - 1].paciente
  } - ${
    pacientes_traumatologia[pacientes_traumatologia.length - 1].prevision
  }.</p>`
);
document.write(
...
var pacientesTotales = radiologia
  .map((consulta) => consulta.paciente)
  .concat(pacientes_traumatologia.map((consulta) => consulta.paciente))
  .concat(dental.map((consulta) => consulta.paciente));

document.write("<h2>Listado de Pacientes del Centro Médico</h2>");
...
var pacientes_isapre = dental.filter(
  (consulta) => consulta.prevision === "ISAPRE"
);
document.write("<h2>Listado de Pacientes de ISAPRE en Dental</h2>");
pacientes_isapre.forEach((consulta) => {
  document.write(
    `<div class="isapre"><ul><li>${consulta.paciente} - ${consulta.prevision}</li></ul></div>`
  );
});

//6. Filtrar aquellos pacientes que indican ser de FONASA en la lista de consultas médicas de Traumatología

var pacientes_fonasa = pacientes_traumatologia.filter(
  (consulta) => consulta.prevision === "FONASA"
);
document.write("<h2>Listado de Pacientes de FONASA en Traumatología</h2>");
pacientes_fonasa.forEach((consulta) => {
  document.write(
    `<div class="isapre"><ul><li>${consulta.paciente} - ${consulta.prevision}</li></ul></div>`
  );
});
